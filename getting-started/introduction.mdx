---
title: "Introduction"
description: "The Senderz API provides programmatic access to Sender ID registration, operator management, and A2P messaging across global markets."
---

## Three APIs, one integration

The Senderz platform exposes three resource namespaces under a single authentication and versioning model:

<CardGroup cols={3}>
  <Card title="Sender ID API" icon="shield-check" href="/sender-id-api/brands-overview">
    Register, manage, and monitor Sender ID registrations across supported markets. Onboard brands, submit SIDs, track approval status.
  </Card>
  <Card title="Operator API" icon="tower-broadcast" href="/operator-api/overview">
    Review, approve, and reject Sender ID registrations for your network. Monitor volumes, approval rates, and processing times.
  </Card>
  <Card title="Messaging API" icon="message" href="/messaging-api/overview">
    Send and track 1-way A2P SMS messages via registered Sender IDs. OTP, logistics notifications, promotional alerts.
  </Card>
</CardGroup>

CPaaS providers, aggregators, MNOs, and enterprise customers can use one or more namespaces depending on their integration needs.

## Base URL

All API requests use:

```
https://api.senderz.ai
```

## Quick start

<Steps>
  <Step title="Get your API key">
    Sign in to the [Senderz dashboard](https://senderz.ai) and generate an API key. Use `sk_test_` keys for sandbox testing, `sk_live_` keys for production.
  </Step>
  <Step title="Onboard a brand">
    ```bash
    curl -X POST https://api.senderz.ai/v1/brands \
      -H "Authorization: Bearer sk_test_xxxxxxxxxxxxx" \
      -H "Content-Type: application/json" \
      -d '{
        "name": "TrackFast Logistics Pty Ltd",
        "identifier_type": "ABN",
        "identifier_value": "12345678901",
        "market": "AU",
        "contact": {
          "name": "Sarah Chen",
          "email": "sarah@trackfast.com.au",
          "phone": "+61400123456"
        }
      }'
    ```
  </Step>
  <Step title="Register a Sender ID">
    ```bash
    curl -X POST https://api.senderz.ai/v1/sids \
      -H "Authorization: Bearer sk_test_xxxxxxxxxxxxx" \
      -H "Content-Type: application/json" \
      -d '{
        "sender_id": "TrackFast",
        "brand_id": "brand_f6g7h8i9",
        "market": "AU",
        "use_case": "Dispatch and delivery notifications for e-commerce orders.",
        "match_type": "business_name",
        "match_value": "TrackFast Logistics Pty Ltd"
      }'
    ```
  </Step>
  <Step title="Send a message">
    ```bash
    curl -X POST https://api.senderz.ai/v1/messaging/messages \
      -H "Authorization: Bearer sk_test_xxxxxxxxxxxxx" \
      -H "Content-Type: application/json" \
      -d '{
        "to": "+61400123456",
        "sender_id": "TrackFast",
        "body": "Your order SYD-90421 has been dispatched. Expected delivery: tomorrow by 5pm.",
        "market": "AU"
      }'
    ```
  </Step>
</Steps>

## Key concepts

| Concept | Description |
|---|---|
| **Market** | ISO 3166-1 alpha-2 country code (e.g. `AU`, `SG`, `HK`). Required on every resource. Determines routing and compliance rules. |
| **Brand** | The legal entity that owns Sender IDs. Must be onboarded and verified before SID registration. |
| **Sender ID** | An alphanumeric identifier (2-11 chars) displayed as the message sender. Must be registered on the relevant market register. |
| **Operator** | A mobile network operator (MNO) that reviews and approves Sender ID registrations for its network. |
| **Message** | A single outbound SMS dispatched via a registered Sender ID. |

## Versioning

The API version is embedded in the URL path: `/v1/`. Breaking changes increment the version. Non-breaking additions (new optional fields, new event types) do not.
